		...
		<div class="col-12">
			...
			<div class="card">
				<div class="card-header">
					<div class="row">
						<div class="col-6">
							<h3 class="card-title">{{ __('adminstaticword.PrivacyPolicy') }}</h3>
						</div>
					</div>
				</div>
				<!-- /.card-header -->
				<form id="edit-policy" method="post" action="{{ route('terms.admin.update') }}" autocomplete="off"
					...
					<div class="card-body">
						...
						<div class="row">
							<div class="col-md-12">
								<label class="text-dark">{{ __('adminstaticword.English PrivacyPolicy') }}:
									<span class="text-danger">*</span></label>
								<input type="hidden" name="policy_en" id="detail1">

								<!-- Container for CKEditor toolbar -->
								<div id="toolbar-container"></div>

								<!-- CKEditor content container -->
								<div name="policy_en" id="editor" class="@error('policy') is-invalid @enderror"
									style="border: 1px solid #c4c4c4">
									@if ($terms_policy)
										{!! $terms_policy->getTranslation('policy', 'en', false) !!}
									@else
						...
						<div class="row">
							<div class="col-md-12">
								<label class="text-dark">{{ __('adminstaticword.Arabic PrivacyPolicy') }}:
									<span class="text-danger">*</span></label>
								<input type="hidden" name="policy_ar" id="detail2">

								<!-- Container for CKEditor toolbar -->
								<div id="toolbar-container2"></div>

								<!-- CKEditor content container -->
								<div name="policy_ar" id="editor2" class="@error('policy') is-invalid @enderror"
									style="border: 1px solid #c4c4c4">
									@if ($terms_policy)
										{!! $terms_policy->getTranslation('policy', 'ar', false) !!}
									@else
										{{ __('adminstaticword.Enter') }}
									@endif
								</div>
	...
	document.addEventListener('DOMContentLoaded', (event) => {
		...
		DecoupledEditor
			...
			.then(editor => {
				...
				const form = document.querySelector('#edit-policy');
