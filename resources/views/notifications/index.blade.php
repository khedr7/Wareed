...
@section('content')
		...
		<div class="col-12">
			...
			<div class="card">
				<div class="card-header">
					<div class="row">
						<div class="col-6">
							<h3 class="card-title">{{ __('adminstaticword.Notifications') }}</h3>
						</div>
						<div class="col-6 text-right">
							<a href="{{ route('notifications.admin.create') }}" class="btn btn-sm btn-primary">
								{{ __('adminstaticword.AddNotification') }}</a>
					...
					<table id="example2" class="table table-bordered table-striped">
						...
						<tbody>
							...
							@foreach ($notifications as $notification)
								...
								<tr>
									<td>
										<label for='checkbox{{ $notification->id }}' class="form-check form-check-label">
											<input type='checkbox' form='bulk_delete_form'
												class='check form-check-input filled-in material-checkbox-input' name='checked[]'
												value="{{ $notification->id }}" id='checkbox{{ $notification->id }}'>
											<span class="form-check-sign">
												<span class="check"></span>
											</span>
										</label>
									...
									<td>
										...
										@elseif($notification->to_type == 'provider')
											{{ __('adminstaticword.Providers') }}
										@elseif($notification->to_type == 'user_provider')
											{{ __('adminstaticword.Providers') }} - {{ __('adminstaticword.Users') }}
										@endif
									</td>
									<td>
										<button type="button" data-toggle="modal" data-target="#delete-notification"
											data-notification-id="{{ $notification->id }}" rel="tooltip"
											class="btn btn-danger btn-sm btn-icon del-notification">
	...
	{{-- delete Modal start --}}
	<div class="modal fade bd-example-modal-sm" id="delete-notification" tabindex="-1" role="dialog" aria-hidden="true">
	...
	<div id="bulk_delete" class="delete-modal modal fade" role="dialog">
		<div class="modal-dialog modal-sm">
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<form id="bulk_delete_form" method="post" action="{{ route('notifications.admin.bulkDelete') }}">
						@csrf
						@method('POST')
...
<script>
	...
	$(document).on('click', '.del-notification', function() {
		...
		$('#del-u-form').attr('action', "{{ url('notifications') }}/" + notificationId);
